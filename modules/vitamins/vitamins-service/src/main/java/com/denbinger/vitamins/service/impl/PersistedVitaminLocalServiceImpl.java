 ...
 *
 * <p>
 * All custom service methods should be put in this class. Whenever methods are added, rerun ServiceBuilder to copy their definitions into the <code>com.denbinger.vitamins.service.PersistedVitaminLocalService</code> interface.
 *
 * <p>
 ...
 */
@Component(
	property = "model.class.name=com.denbinger.vitamins.model.PersistedVitamin",
...
public class PersistedVitaminLocalServiceImpl
	 ...
	 * NOTE FOR DEVELOPERS:
	 *
	 * Never reference this class directly. Use <code>com.denbinger.vitamins.service.PersistedVitaminLocalService</code> via injection or a <code>org.osgi.util.tracker.ServiceTracker</code> or use <code>com.denbinger.vitamins.service.PersistedVitaminLocalServiceUtil</code>.
	 */
	...
	public PersistedVitamin addPersistedVitamin(final String id,
												final String name,
		...
		PersistedVitamin vitamin =
				createPersistedVitamin(counterLocalService.increment(
						PersistedVitamin.class.getName()));
		...
		resourceLocalService.addResources(
				vitamin.getCompanyId(), vitamin.getGroupId(),
				vitamin.getUserId(),
				PersistedVitamin.class.getName(),
		...
		if ((chemicalNames != null) && (chemicalNames.length > 0)) {
			for (String value : chemicalNames) {
				vitaminDetailLocalService.addVitaminDetail(
						vitamin.getPersistedVitaminId(),
		...
		if ((properties != null) && (properties.length > 0)) {
			for (String value : properties) {
				vitaminDetailLocalService.addVitaminDetail(
						vitamin.getPersistedVitaminId(),
						VitaminDetailType.PROPERTY, value, serviceContext);
			}
		}
		if ((attributes != null) && (attributes.length > 0)) {
			for (String value : attributes) {
				vitaminDetailLocalService.addVitaminDetail(
						vitamin.getPersistedVitaminId(),
						VitaminDetailType.ATTRIBUTE, value, serviceContext);
			}
		}
		if ((symptoms != null) && (symptoms.length > 0)) {
			for (String value : symptoms) {
				vitaminDetailLocalService.addVitaminDetail(
						vitamin.getPersistedVitaminId(),
						VitaminDetailType.SYMPTOM, value, serviceContext);
			}
		}
		if ((risks != null) && (risks.length > 0)) {
			for (String value : risks) {
				vitaminDetailLocalService.addVitaminDetail(
						vitamin.getPersistedVitaminId(),
						VitaminDetailType.RISK, value, serviceContext);
			}
		}
	...
	public PersistedVitamin updatePersistedVitamin(final String oldId,
		...
		PersistedVitamin vitamin =
				persistedVitaminPersistence.fetchBySurrogateId(oldId);
		...
		if ((id == null) || (id.trim().length() < 1)) {
			useId = RandomStringUtils
					.random(10, true, true).toUpperCase();
		} else {
			useId = id.trim().toUpperCase();
		}
		...
		if ((chemicalNames != null) && (chemicalNames.length > 0)) {
			for (String value : chemicalNames) {
				vitaminDetailLocalService.addVitaminDetail(
						vitamin.getPersistedVitaminId(),
						VitaminDetailType.CHEMICAL_NAME, value, serviceContext);
			}
		}
		if ((properties != null) && (properties.length > 0)) {
			for (String value : properties) {
	...
	@Indexable(type = IndexableType.REINDEX)
	@SystemEvent(type = SystemEventConstants.TYPE_DEFAULT)
	public PersistedVitamin patchPersistedVitamin(final String oldId,
												  final String id,
												  final String name,
												  final String groupName,
												  final String description,
		...
		if ((chemicalNames != null) && (chemicalNames.length > 0)) {
			vitaminDetailLocalService.deleteVitaminDetailsByType(
					vitamin.getPersistedVitaminId(),
					VitaminDetailType.CHEMICAL_NAME);
			changed = true;
			for (String value : chemicalNames) {
	...
	public PersistedVitamin deleteVitamin(PersistedVitamin persistedVitamin) {
		try {
			resourceLocalService.deleteResource(
					persistedVitamin.getCompanyId(),
					PersistedVitamin.class.getName(),
					ResourceConstants.SCOPE_INDIVIDUAL,
					persistedVitamin.getPersistedVitaminId());
		} catch (PortalException e) {
			_log.warn("Error deleting persisted vitamin permissions: " +
					e.getMessage(), e);
		}

		vitaminDetailLocalService.deleteAllVitaminDetails(
				persistedVitamin.getPersistedVitaminId());
	...
	@Reference(unbind = "~")
	protected void setVitaminDetailLocalService(
			final VitaminDetailLocalService vitaminDetailLocalService) {
	...
	private static final Logger _log =
			LoggerFactory.getLogger(PersistedVitaminLocalServiceImpl.class);
}